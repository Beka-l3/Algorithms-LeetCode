...
///   Умножение и Суммирование СТОЛБИКОМ
///   при умножений, результат может быть максимум `arr.count ~ a.count + b.sount`
///   `arr[i + j + 1] = a[i] * b[j]`
///   после, идет суммирование с `carry`
///   тут `carry` может быть больше `1`
///   и нужно убрать лишние нули из начала
